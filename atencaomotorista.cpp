...
int main() {
    ...
    if(lgGpioClaimOutput(h_gpio, 0, PIN_LED_SONOLENCIA, 0) < 0) return -1;
    if(lgGpioClaimOutput(h_gpio, 0, PIN_LED_BOCEJO, 0) < 0) return -1;
    if(lgGpioClaimOutput(h_gpio, 0, PIN_BUZZER, 0) < 0) return -1;
    if(lgGpioClaimOutput(h_gpio, 0, PIN_LED_FALHA, 0) < 0) return -1;
    ...
    const double HDR_DROP_FACTOR = 0.85; // Multiplicador para queda da cabeca
    ...
    const int HEAD_CONSECUTIVE_FRAMES = 25; 
    ...
    cv::Mat frame, gray;
    std::cout << "Monitor iniciado..." << std::endl;
        ...
        if(faces.empty()) {
            ...
            if(success) {
                ...
                if(is_calibrating) {
                    ...
                    // Impressao dos valores na tela (HUD)
